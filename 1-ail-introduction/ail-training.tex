...
\begin{frame}
    \frametitle{Splash manager}
    	\begin{itemize}
            \item Splash manager\footnote{\url{https://github.com/ail-project/ail-splash-manager}} is a standalone software\footnote{Crawlers can installed and executed outside AIL} to run the containers for crawling
		    \item AIL crawlers are using Splash dockers to fetch and render domains
		    \item Manage and control Splash Crawlers
			\begin{itemize}
				\item Install and run Splash crawlers on another server,
				\item Handle proxies (Web and Tor by default),
				\item Launch/Kill Splash Dockers automatically,
				\item Restart crawlers if these crash,
			\end{itemize}
        \end{itemize}
\end{frame}

\begin{frame}
	\frametitle{Splash manager - UI in AIL}
		\begin{figure}
		    \includegraphics[scale=0.185]{images/ail_splash_manager.png}
		\end{figure}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item[] \textbf{Databases:} Redis and ARDB\footnote{We are migrating to kvrocks}
...
\begin{frame}
    ...
    \begin{itemize}
            ...
            \item Semantic aspects (task with Corexalys) are challenging due to the diverse data sources, unstructured data and languages seen.
    \end{itemize}
\end{frame}
...
\begin{frame}
    \frametitle{Ongoing developments}
        \begin{itemize}
            ...
            \item Extension of the tracker with typo-squatting library
