...
\author{\Large{Alexandre Dulaunoy}\\ \scriptsize{alexandre.dulaunoy@circl.lu}\\ \large{Aurélien Thirion}\\ \scriptsize{aurelien.thirion@circl.lu}\\ \large{Sami Mokaddem}\\ \scriptsize{sami.mokaddem@circl.lu}\\}
...
\frametitle{Links}
    \begin{itemize}
        ...
        \item Slide deck: \url{https://github.com/ail-project/ail-training}
        \item gitter: TODO
    \end{itemize}
\end{frame}
...
"AIL is a modular framework to analyse potential information leaks from unstructured data sources."
    ...
    \begin{tikzpicture}[scale=1.0]
        ...
        \node[simplenode] (item) at (0, 0) {\includegraphics[scale=0.20]{images/file_text_icon.png}};
        ...
        \begin{scope}
            \draw[flux] (item.east) to (ail.west);
        \foreach \i in {-2,...,-1}{% 
            \pgfmathsetlengthmacro{\radius}{(sin(atan(\i*0.1))+0.1)*1cm}
            %\draw[->] ([yshift=\i * 0.2 cm]item.east) to [out=-50,in=-130] ([yshift=\i * 0.2 cm]ail.west) ;}
            %\draw[->] ([xshift=\radius,yshift=\i * 0.2 cm]item.east) to [out=-10+\i*10,in=-130] ([yshift=\i * 0.2 cm]ail.west) ;}
            \draw[flux] ([xshift=\radius,yshift=\i * 0.2 cm]item.east) to [out=-10+\i*10,in=-130] ([xshift=-\i*0.05cm,yshift=\i * 0.2 cm]ail.west) ;}
    ...
    \frametitle{Ongoing developments}
        \begin{itemize}
            ...
            \begin{itemize}
            	...
            	\item tor indexer
            	\item {\bf New crawler and proxy manager (coming in one or two week) }
            \end{itemize}
        \end{itemize}
\end{frame}
