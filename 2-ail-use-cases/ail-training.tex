...
\author{\large{Aurelien Thirion}\\ \scriptsize{aurelien.thirion@circl.lu}}
\title{AIL Project}
\subtitle{Open source framework to efficiently collect, crawl, dig, and analyze unstructured data}
...
\begin{itemize}
\item Demonstrate the practical usage and extensibility of an open source tool for monitoring web pages, pastes, forums, and hidden services
\item Discuss the challenges involved and delve into the design principles of the AIL open source framework
\item Explore various {\bf collection mechanisms and sources utilized} by the AIL framework
\item Gain knowledge on creating new modules within the AIL framework
\item Acquire (quickly) proficiency in using, installing, and initializing AIL
\item Understand the significance of integrating the AIL framework into the cyber threat intelligence life cycle, with notable tools such as MISP
\end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item The AIL framework is an \textbf{open source software} in Python. The software is actively used (and maintained) by CIRCL and many organisations.
        \item In 2020, AIL framework became a complete project called \textbf{ail project}\footnote{\url{https://github.com/ail-project/}}.
        \item In 2023, AIL framework version 5.0 released with a new datastorage back-end.
        \item In 2023, AIL framework version 5.3 released with a new IM crawl functionality.
    \end{itemize}
\end{frame}
...
\begin{frame}
    \frametitle{Common usage}
        \begin{itemize}
                ...
                \item {\bf Search} for leaks inside large leak archive
                \item {\bf Monitor} and crawl websites
        \end{itemize}
\end{frame}

\begin{frame}
\frametitle{Supporting CERT and Law Enforcement Activities}
\begin{itemize}
\item Proactive Investigation: Detection of Leaks
\begin{itemize}
	\item Compilation of leaked emails and passwords
	\item Analysis of leaked databases
    \item Identification of exposed SaaS keys (AWS, Google,...)
	\item Detection of compromised credit card information
	\item Identification and analysis of compromised PGP private keys and certificate keys
\end{itemize}
	\item Contributing to Passive DNS and Metadata Collection Systems
	\item Sharing CVEs and Proof-of-Concepts (PoCs) for commonly exploited vulnerabilities
	\item Deanonymization of Hidden Services
\end{itemize}
\end{frame}
    ...
    \frametitle{Support CERT and Law Enforcement activities}
    	\begin{itemize}
		    \item Website monitoring
		    	\begin{itemize}
				    \item Monitor booters, marketplaces, forums
                    \item Detect encoded exploits (WebShell, malware encoded in Base64,...)
				    \item SQL injections
		    	\end{itemize}
		    \item Automatic and manual submission to threat intelligence sharing and incident
response platforms
			\begin{itemize}
				\item MISP
				\item TheHive
			\end{itemize}
		    \item Term/Regex/YARA monitoring for local companies/government keywords
        \end{itemize}
\end{frame}
...
\begin{frame}
    \frametitle{Catching mistakes from users}
        \begin{figure}
...
\begin{frame}
    \frametitle{Example - Sources of leaks - paste monitoring}
    \begin{itemize}
...
\begin{frame}
\frametitle{Purposes of Leaks}
\begin{itemize}
\item {\bf Economic Interests}: Adversaries may promote services for their own financial gain.
\item {\bf Ransom Model}: Leaks can be used to publicly pressure victims into meeting certain demands.
\item {\bf Political Motives}: Adversaries may leak information to showcase their power or influence.
\item {\bf Collaboration}: Criminals may need to collaborate and share leaked information for their operations.
\item {\bf Operational Infrastructure}: Examples include malware that exfiltrates information to pastie websites.
\item {\bf Mistakes and Errors}: Leaks can also occur due to unintentional mistakes or errors.
\end{itemize}
\end{frame}

%\begin{frame}
%    \frametitle{Objectives for SOC/CSIRT teams}
%    \begin{itemize}
...
\frametitle{Objectives for SOC/CSIRT Teams}
\begin{itemize}
\item \textbf{Contacting Companies or Organizations}: Reach out to companies or organizations responsible for specific accidental leaks to address the issue
\item \textbf{Engaging with Media}: Collaborate with the media to discuss specific leak cases and find practical ways to increase factual information available to the public
\item Evaluate the Cybercriminal Economy: Analyze the cybercriminal market, including activities such as DDoS booters\footnote{\url{https://github.com/D4-project/}} and the reselling of personal information, in order to understand the disparity between reality and media coverage
\item Analyze the Collateral Effects: Investigate the broader impact of malware, software vulnerabilities, or data exfiltration incidents
\end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Automatic Tor Crawler and website crawling (handle cookies authentication) via Lacus\footnote{\url{https://github.com/ail-project/lacus}}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item PGP, Cryptocurrencies, Decoded (Base64, ...) and username Correlation
        \item And many more
    \end{itemize}
\end{frame}
...
\begin{figure}[h] 
    ...
    %styles 
    \tikzstyle{Lacus}=[ellipse,draw,align=center]
    \tikzstyle{AIL}=[rectangle,draw, align=center, fill={rgb:red,1;green,2;blue,5}] 
    \tikzstyle{playwright}=[rectangle,draw, align=center, fill={rgb:red,1;green,2;blue,5}]
...
\begin{frame}
    \frametitle{Auto Crawler}
    \begin{itemize}
    \item[] How a domain is crawled by default
		\begin{enumerate}
		    \item Fetch the first url
            \item Render the {\bf web page including javascript} (done by playwright via Lacus)
...
\begin{frame}
\frametitle{Lacus: Web Capturing System}
\begin{itemize}
\item Lacus\footnote{\url{https://github.com/ail-project/lacus}} is a web capturing system built on playwright.
\item AIL utilizes Lacus for fetching and rendering domains.
\begin{itemize}
\item Lacus can be installed and used independently from AIL.
\item Capture what you need by enqueuing requests.
\item Initiate the capture process.
\item Retrieve the capture results.
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{figure}
        \includegraphics[scale=0.45, angle=0]{screenshot/ail-objs-legend-1.png}
        \includegraphics[scale=0.5, angle=0]{screenshot/ail-objs-legend-2.png}
    \end{figure}
\end{frame}
    ...
    \begin{enumerate}
        \item AIL Importers:
            \begin{itemize}
                ...
                \item \textit{pystemon}
            \end{itemize}
        \item AIL Feeders (discord, telegram, ActivityPub, ...)
...
\begin{frame}
\frametitle{Feeding Data to AIL - Technical Considerations}
\begin{itemize}
\item It is important to consider the size of each file being fed into AIL:
\begin{itemize}
\item For optimal processing and efficiency, it is recommended to keep each file around 3 MB in size
\item This balance between processing capabilities and file size is crucial, as certain modules perform various computations, such as regexp matching, which has a default timeout of 30 seconds
\item If you need to process a large file, it is advisable to split it into multiple smaller files. The AIL leak feeder tool\footnote{\url{https://github.com/ail-project/ail-feeder-leak}} can assist you in this task.
\end{itemize}
\end{itemize}
\end{frame}
...
\frametitle{Contact}
\begin{itemize}
\item CIRCL has developed a range of open-source tools for intelligence analysts and incident responders.
\item We welcome partnerships and collaboration discussions. Feel free to contact us\footnote{\url{mailto:info@circl.lu}}.
\end{itemize}
\end{frame}
